...
depends_on: []
files_modified:
  - bot/src/config/chains/arbitrum-sepolia.ts  # conditional: only if deltaThresholdPercent needs lowering
autonomous: false
...
<task type="auto">
  ...
  <files>bot/src/config/chains/pools/arbitrum-sepolia.ts</files>
  <action>
Run pre-flight checks using cast against the live Arbitrum Sepolia RPC. Load env first: `set -a && source .env.arbitrum-sepolia && set +a`
...
- If NodeInterface fails: RPC endpoint may not support the precompile. Try a different RPC provider.
- If slot0 reverts: Pool may have been destroyed or redeployed. Use `cast call $FACTORY "getPool(address,address,uint24)(address)" $WETH $USDC $FEE` to re-discover.
  ...
  <verify>All four checks return non-error responses with valid data.</verify>
  <done>Pre-run infrastructure verified: both pools have liquidity, FlashloanExecutor deployed, NodeInterface responds, slot0 readable.</done>
</task>
...
<task type="auto">
  ...
  <files></files>
  <action>
Validate that `eth_call` simulation works on Arbitrum Sepolia against the deployed FlashloanExecutor. Load env first: `set -a && source .env.arbitrum-sepolia && set +a`
...
- No "connection refused" or "method not found" errors appear (RPC supports eth_call)
  ...
  <verify>Test A returns deployer address. Tests B and/or C produce revert errors (not connection errors).</verify>
  <done>eth_call simulation verified: view functions return correct data, reverts propagate correctly on Arbitrum Sepolia.</done>
</task>
