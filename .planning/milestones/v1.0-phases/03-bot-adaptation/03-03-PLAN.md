...
must_haves:
  ...
  artifacts:
    ...
    - path: "bot/src/reporting.ts"
      ...
      contains: "l1DataFee"
    - path: "bot/__tests__/gas/ArbitrumGasEstimator.test.ts"
      provides: "Unit tests for ArbitrumGasEstimator"
      contains: "estimateArbitrumGas"
    - path: "bot/__tests__/config/chain-config.test.ts"
...
<task type="auto">
  ...
  <action>
    ...
    **In `bot/src/run-arb-sepolia.ts`:**
    ...
    9. Pass `gasEstimatorFn: arbGasEstimator` in the detector config when constructing FlashloanBot:
       ...
       **Resolution:** The entry point should NOT pass `gasEstimatorFn` through `BotConfig`. Instead, add a `setGasEstimator(fn)` public method to `OpportunityDetector` that stores the function and changes the delta handling dispatch. After constructing the bot, inject the gas estimator:
       ```typescript
       // In OpportunityDetector, add:
  ...
  <verify>
    ...
    3. `OpportunityDetector` has public `setGasEstimator(fn)` method that stores the gas estimator function.
...
<task type="auto">
  ...
  <action>
    **Create `bot/__tests__/gas/ArbitrumGasEstimator.test.ts`:**
    ...
    **Create `bot/__tests__/config/chain-config.test.ts`:**
  ...
  <verify>
    ...
    3. Run `pnpm test -- --run bot/__tests__/gas/ArbitrumGasEstimator.test.ts` — all pass.
    4. Run `pnpm test -- --run bot/__tests__/config/chain-config.test.ts` — all pass.
  </verify>
  <done>
