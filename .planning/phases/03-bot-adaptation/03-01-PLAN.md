...
<task type="auto">
  ...
  <action>
    ...
    **Update `package.json`:**
    ...
    8. Add a new script: `"bot:arb-sepolia": "node --loader ts-node/esm bot/src/run-arb-sepolia.ts"`. Place it next to the existing `bot:testnet` script. If `bot:testnet` doesn't exist, add it alongside whatever bot-related scripts exist.
