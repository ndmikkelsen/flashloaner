...
<task type="checkpoint:human-action" gate="blocking">
  <name>Task 2: Fund deployer wallet and set environment variables</name>
  <action>
Ensure the deployer wallet has sufficient ETH for deployment gas on Arbitrum and set all required environment variables in your terminal session.

**1. Check deployer wallet balance:**
```bash
cast balance 0x8d7a596F072e462E7b018747e62EC8eB01191a18 --rpc-url $ARBITRUM_RPC_URL
```
You need approximately 0.01-0.05 ETH for deployment (Arbitrum gas is cheap). Fund the wallet from an exchange or transfer from another wallet if needed.

**2. Get an Arbiscan API key** (required for contract verification):
- Visit https://arbiscan.io/myapikey
- Create a free account if needed, generate an API key

**3. Set ALL required environment variables in your terminal session (do NOT write to any committed file):**
```bash
# Deployment credentials
export DEPLOYER_PRIVATE_KEY="<your deployer private key — hex, 0x-prefixed>"
export ARBISCAN_API_KEY="<your Arbiscan API key>"

# Bot wallet — must match the account that DEPLOYER_PRIVATE_KEY controls
export BOT_WALLET_ADDRESS="0x8d7a596F072e462E7b018747e62EC8eB01191a18"

# Arbitrum protocol addresses (mainnet — these are fixed, not secrets)
...
export MIN_PROFIT_WEI="10000000000000000"      # 0.01 ETH minimum profit
export MAX_GAS_PRICE="100000000000"            # 100 gwei safety cap
export MAX_TRADE_SIZE="1000000000000000000000" # 1000 ETH max flash loan
export FAILURE_THRESHOLD="5"                   # Circuit breaker threshold
```

**Critical notes:**
- DEPLOYER_PRIVATE_KEY address MUST match BOT_WALLET_ADDRESS. The `onlyAuthorized` modifier on FlashloanExecutor checks this. If they don't match, all bot calls revert.
- UNISWAP_V2_ROUTER is NOT set — Uniswap V2 was never deployed on Arbitrum. The deploy script handles this gracefully (skips UniswapV2Adapter).
- SUSHISWAP_V3_ROUTER is NOT set — Sushi V3 on Arbitrum lacks a known router address. Skipped.
- ADAPTER_RAMSES env var is not needed at deployment — RamsesV2Adapter exists but is not wired in Deploy.s.sol (no active Ramses pools).
  </action>
  <verify>
Run `echo $DEPLOYER_PRIVATE_KEY | wc -c` — should output 67 (length of 0x-prefixed hex private key + newline). Run `cast balance $BOT_WALLET_ADDRESS --rpc-url $ARBITRUM_RPC_URL` — should show balance > 0.01 ETH. Run `echo $ARBISCAN_API_KEY` — should show a non-empty string.
  </verify>
  <done>Deployer wallet funded with more than 0.01 ETH on Arbitrum mainnet. All 14 environment variables exported in the terminal session. DEPLOYER_PRIVATE_KEY controls the same address as BOT_WALLET_ADDRESS.</done>
</task>
