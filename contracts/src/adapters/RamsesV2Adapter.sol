...
/// @notice Minimal Ramses V3 SwapRouter interface (identical to Uniswap V3).
...
/// @notice Minimal Ramses V3 QuoterV2 interface for off-chain quoting (identical to Uniswap V3).
interface IQuoterV2 {
...
/// @title RamsesV2Adapter
/// @notice DEX adapter for Ramses V3 swaps with fee tier support.
/// @dev Implements IDEXAdapter for single-hop and multi-hop Ramses V3 swaps.
///      Ramses V3 is a Uniswap V3 fork with identical ABI.
///
///      extraData encoding:
...
contract RamsesV2Adapter is IDEXAdapter {
    using SafeERC20 for IERC20;

    /// @notice Fee tier not valid for Ramses V3.
    error InvalidFeeTier(uint24 fee);

    /// @notice Fees and intermediates array length mismatch.
    error PathLengthMismatch();

    /// @notice The Ramses V3 SwapRouter.
    ISwapRouter public immutable swapRouter;

    /// @notice The Ramses V3 QuoterV2 for off-chain quotes.
    ...
    /// @param _swapRouter The Ramses V3 SwapRouter address
    /// @param _quoter The Ramses V3 QuoterV2 address
        ...
        returns (uint256 amountOut)
    {
        ...
        require(success, "RamsesV2Adapter: quote failed");
    ...
    /// @dev Execute a single-hop Ramses V3 swap.
    ...
    /// @dev Execute a multi-hop Ramses V3 swap.
    ...
    /// @dev Encode a Ramses V3 multi-hop path: token0 + fee0 + token1 + fee1 + ... + tokenN.
    ...
    /// @return path The packed Ramses V3 path bytes
